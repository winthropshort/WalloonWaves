...
#   [W2] GET /weather/locations returns 7 locations
...
if [ -n "$API_URL" ]; then
  ...
  check "W2" "GET /weather/locations returns 7 locations" \
    bash -c "curl -sf --max-time 10 '${API_URL}weather/locations' | python3 -c \
      'import sys,json; d=json.load(sys.stdin); assert d[\"success\"] and len(d[\"data\"])==7'"
